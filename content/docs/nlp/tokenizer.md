...
Cons:
- Ignore context $\rightarrow$ suboptimal splits for words with different meanings in different contexts
...
Cons:
- Only save the final vocabulary, not the merge rules $\rightarrow$ label an entire word as [UNK] when any part is not in the vocabulary
- Ignore context $\rightarrow$ suboptimal splits for words with different meanings in different contexts
